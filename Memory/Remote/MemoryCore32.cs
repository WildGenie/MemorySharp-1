...
namespace Binarysharp.MemoryManagement.Memory.Remote
{
    ...
    public class MemoryCore32 : IMemoryCore
    {
            ...
            MemoryAllocationFlags allocationFlags = MemoryAllocationFlags.Commit)
        {
            ...
            var ret = SafeNativeMethods.VirtualAllocEx(processHandle, IntPtr.Zero, size, allocationFlags,
                protectionFlags);
            ...
            MemoryProtectionFlags protection)
        {
            ...
            if (SafeNativeMethods.VirtualProtectEx(processHandle, address, size, protection, out oldProtection))
            {
        ...
        public void FreeMemory(SafeMemoryHandle processHandle, IntPtr address)
        {
            ...
            // Free the memory
            if (!SafeNativeMethods.VirtualFreeEx(processHandle, address, 0, MemoryReleaseFlags.Release))
            {
        ...
        public SafeMemoryHandle OpenProcess(ProcessAccessFlags accessFlags, int processId)
        {
            ...
            var handle = SafeNativeMethods.OpenProcess(accessFlags, false, processId);
        ...
        public byte[] ReadBytes(SafeMemoryHandle processHandle, IntPtr address, int size)
        {
            ...
            if (SafeNativeMethods.ReadProcessMemory(processHandle, address, buffer, size, out nbBytesRead) &&
        ...
        public MemoryBasicInformation QueryInformationMemory(SafeMemoryHandle processHandle, IntPtr baseAddress)
        {
            ...
            if (
                SafeNativeMethods.VirtualQueryEx(processHandle, baseAddress, out memoryInfo,
            ...
            IntPtr addressFrom, IntPtr addressTo)
        {
            ...
            do
            {
                ...
                ret = SafeNativeMethods.VirtualQueryEx(processHandle, new IntPtr(numberFrom), out memoryInfo,
        ...
        public ProcessBasicInformation QueryInformationProcess(SafeMemoryHandle processHandle)
        {
            ...
            var ret = SafeNativeMethods.NtQueryInformationProcess(processHandle,
        ...
        public int WriteBytes(SafeMemoryHandle processHandle, IntPtr address, byte[] byteArray)
        {
            ...
            if (SafeNativeMethods.WriteProcessMemory(processHandle, address, byteArray, byteArray.Length,
                out nbBytesWritten))
            {
