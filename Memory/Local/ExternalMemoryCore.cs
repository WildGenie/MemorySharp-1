...
namespace Binarysharp.MemoryManagement.Memory.Local
{
    ...
    public static class ExternalMemoryCore
    {
        ...
        {
            var bytes = new byte[SafeMarshalType<T>.Size];
            bytes = ReadProcessMemory(processHandle, address, bytes.Length);
            if (UnsafeMarshalType<T>.IsIntPtr)
            {
                object ret;
            ...
            return SafeMarshalType<T>.ByteArrayToObject(bytes);
        }
        ...
        public static T[] ReadArray<T>(IntPtr processHandle, IntPtr address, int count) where T : struct
        {
            ...
            {
                array[i] = Read<T>(processHandle, address + SafeMarshalType<T>.Size*i);
            }
            return array;
        }
        ...
        {
            var bytes = SafeMarshalType<T>.ObjectToByteArray(value);
        ...
        public static void WriteArray<T>(IntPtr processHandle, IntPtr address, T[] dataArray) where T : struct
        {
            ...
            {
                Write(processHandle, address + SafeMarshalType<T>.Size*i, dataArray[i]);
            }
        }
        ...
        {
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to the ExternalMemoryCore.Allocate method was invalid.");
        ...
        public static MemoryBasicInformation Query(IntPtr processHandle, IntPtr baseAddress)
        {
            ...
            // Check if the handles are valid
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to ExternalMemoryCore.Query method was invalid.");

            // Query the memory region
            if (
                UnsafeNativeMethods.VirtualQueryEx(processHandle, baseAddress, out memoryInfo,
                    SafeMarshalType<MemoryBasicInformation>.Size) != 0)
                return memoryInfo;
            ...
            IntPtr addressTo)
        {
            ...
            do
            {
                ...
                ret = UnsafeNativeMethods.VirtualQueryEx(processHandle, new IntPtr(numberFrom), out memoryInfo,
                    SafeMarshalType<MemoryBasicInformation>.Size);
