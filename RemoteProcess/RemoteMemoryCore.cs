...
namespace Binarysharp.MemoryManagement.RemoteProcess
{
    /// <summary>
    ...
    ///     </remarks>
    /// </summary>
    public static class RemoteMemoryCore
    {
        /// <summary>
        ...
        public static byte[] ReadProcessMemory(IntPtr processHandle, IntPtr address, int size)
        {
            ...
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to the ReadProcessMemory method was invalid.");
        ...
        public static unsafe T Read<T>(IntPtr processHandle, IntPtr address) where T : struct
        {
            var bytes = new byte[RemoteMarshal<T>.Size];
            bytes = ReadProcessMemory(processHandle, address, bytes.Length);
            if (LocalMarshal<T>.IsIntPtr)
            {
                object ret;
            ...
            return RemoteMarshal<T>.ByteArrayToObject(bytes);
        }
        ...
        public static T[] ReadArray<T>(IntPtr processHandle, IntPtr address, int count) where T : struct
        {
            ...
            {
                array[i] = Read<T>(processHandle, address + RemoteMarshal<T>.Size*i);
            }
            return array;
        }
        ...
        public static int WriteProcessMemory(IntPtr processHandle, IntPtr address, byte[] byteArray)
        {
            // Check if the handles are valid
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to the WriteProcessMemory method was invalid.");
        ...
        public static void Write<T>(IntPtr processHandle, IntPtr address, T value) where T : struct
        {
            var bytes = RemoteMarshal<T>.ObjectToByteArray(value);
        ...
        /// <param name="isRelative">[Optional] State if the address is relative to the main module.</param>
        public static void WriteArray<T>(IntPtr processHandle, IntPtr address, T[] dataArray) where T : struct
        {
            // Write the array in the remote process
            for (var i = 0; i < dataArray.Length; i++)
            {
                Write(processHandle, address + RemoteMarshal<T>.Size*i, dataArray[i]);
            }
        }
        ...
        {
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to the RemoteMemory.Allocate method was invalid.");
        ...
        {
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to the NtQueryInformationProcess method was invalid.");
        ...
        public static MemoryBasicInformation Query(IntPtr processHandle, IntPtr baseAddress)
        {
            ...
            // Check if the handles are valid
            if (processHandle == IntPtr.Zero)
                throw new Exception("The handle passed to RemoteMemory.Query method was invalid.");

            // Query the memory region
            if (
                UnsafeNativeMethods.VirtualQueryEx(processHandle, baseAddress, out memoryInfo,
                    RemoteMarshal<MemoryBasicInformation>.Size) != 0)
                return memoryInfo;
            ...
            IntPtr addressTo)
        {
            ...
            do
            {
                ...
                ret = UnsafeNativeMethods.VirtualQueryEx(processHandle, new IntPtr(numberFrom), out memoryInfo,
                    RemoteMarshal<MemoryBasicInformation>.Size);
