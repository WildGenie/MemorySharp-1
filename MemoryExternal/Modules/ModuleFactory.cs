...
namespace Binarysharp.MemoryManagement.MemoryExternal.Modules
{
    ...
    public class ModuleFactory : IFactory
    {
        ...
        /// </summary>
        public IEnumerable<InjectedModule> InjectedModules => InternalInjectedModules;
            ...
            foreach (
                var cachedFunction in
                    RemoteModule.CachedFunctions.ToArray()
                        .Where(cachedFunction => cachedFunction.Key.Item2 == MemorySharp.Handle))
            {
        ...
        public void Eject(InjectedModule module)
        {
            // If the module is valid
            if (!module.IsValid) return;

            // Find if the module is an injected one
            var injected = InternalInjectedModules.FirstOrDefault(m => m == module);
            if (injected.IsNotNull())
