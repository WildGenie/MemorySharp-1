...
namespace Binarysharp.MemoryManagement.Internals
{
    ...
    public static class UnsafeMarshalType<T>
    {
        #region Constructors

        static UnsafeMarshalType()
        {
            ...
            Debug.Write("[SafeMarshalType] " + typeof (T) + " Size: " + SizeU);
            ...
            var method = new DynamicMethod($"GetPinnedPtr<{typeof (T).FullName.Replace(".", "<>")}>",
                typeof (void*), new[] {typeof (T).MakeByRefType()}, typeof (UnsafeMarshalType<>).Module);
