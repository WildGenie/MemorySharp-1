...
namespace MemorySharp.Patterns
{
    ...
    public class PatternFactory : IFactory
    {
        ...
        public PatternFactory(MemoryBase memorySharp)
        {
            _memory = memorySharp;
        }
        #endregion
            ...
            bool reBase)
        {
            ...
            for (var offset = 0; offset < patternData.Length; offset++)
            {
                ...
                result.OriginalAddress = _memory.Read<IntPtr>(_memory.BaseAddress + offset + offsetToAdd);
                result.Address = IntPtr.Subtract(result.OriginalAddress, (int) _memory.BaseAddress);
                result.Offset = offset;
                if (!isOffsetMode)
                {
                    result.Address = reBase ? (_memory.BaseAddress + (int) result.Address) : result.Address;
                    return result;
                }
                result.Offset = reBase ? (int) (_memory.BaseAddress + offset) : result.Offset;
