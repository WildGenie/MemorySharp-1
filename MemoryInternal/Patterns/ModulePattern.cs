...
namespace Binarysharp.MemoryManagement.MemoryInternal.Patterns
{
    /// <summary>
    /// </summary>
    public class ModulePattern : IFactory
    {
        #region  Fields
        private readonly MemoryPlus _memory;
        private readonly RemoteModule _module;
        ...
        #region Constructors
        /// <summary>
        ///     Initializes a new instance of the <see cref="ModulePattern" /> class.
        /// </summary>
        /// <param name="memorySharp">The <see cref="MemorySharp" /> instance.</param>
        public ModulePattern(RemoteModule module)
        {
            _module = module;
        }
        #endregion
        ...
        public byte[] MainModuleData
        {
            get
            {
                return _moduleData ??
                       (_moduleData =
                           _memory.ReadBytes(_module.BaseAddress,
                               _module.Size));
            }
            ...
            bool reBase)
        {
            ...
            for (var offset = 0; offset < patternData.Length; offset++)
            {
                ...
                result.OriginalAddress = _memory.Read<IntPtr>(_memory.Process.ImageBase + offset + offsetToAdd);
                result.Address = IntPtr.Subtract(result.OriginalAddress, (int) _memory.Process.ImageBase);
                result.Offset = offset;
                if (!isOffsetMode)
                {
                    result.Address = reBase ? (_memory.Process.ImageBase + (int) result.Address) : result.Address;
                    return result;
                }
                result.Offset = reBase ? (int) (_memory.Process.ImageBase + offset) : result.Offset;
