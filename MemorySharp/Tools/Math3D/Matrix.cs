...
namespace Binarysharp.MemoryManagement.Tools.Math3D
{
    /// <summary>
    ...
    public struct Matrix
    {
        ...
        {
            var data = new byte[Data.Length*MarshalType<float>.Size];
            for (var i = 0; i < Data.Length; i++)
            {
                Array.Copy(BitConverter.GetBytes(Data[i]), 0, data, i*MarshalType<float>.Size, MarshalType<float>.Size);
            }
            return data;
        }
    }
