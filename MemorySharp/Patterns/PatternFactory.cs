...
namespace Binarysharp.MemoryManagement.Patterns
{
    ...
    public class PatternFactory : IFactory
    {
        ...
        {
            get { return _moduleData ?? (_moduleData = _memory.ReadBytes(_memory.ImageBase, _memory.ImageSize)); }
        ...
        public PatternFactory(MemorySharp memorySharp)
        {
            _memory = memorySharp;
        }
        #endregion
            ...
            bool reBase)
        {
            ...
            for (var offset = 0; offset < patternData.Length; offset++)
            {
                ...
                result.OriginalAddress = _memory.Read<IntPtr>(_memory.ImageBase.Add(offset + offsetToAdd));
                result.Address = IntPtr.Subtract(result.OriginalAddress, (int) _memory.ImageBase);
                result.Offset = offset;
                if (!isOffsetMode)
                {
                    result.Address = reBase ? (_memory.ImageBase.Add(result.Address)) : result.Address;
                    return result;
                }
                result.Offset = reBase ? (int) (_memory.ImageBase.Add(offset)) : result.Offset;
