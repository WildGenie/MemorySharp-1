...
namespace Binarysharp.MemoryManagement.Memory
{
    ...
    public static class ExternalMemoryCore
    {
        ...
        {
            var bytes = new byte[MarshalType<T>.Size];
            ...
            return MarshalType<T>.ByteArrayToObject(bytes);
        }
        ...
        {
            var bytes = MarshalType<T>.ObjectToByteArray(value);
        ...
        public static void WriteArray<T>(IntPtr processHandle, IntPtr address, T[] dataArray)
        {
            ...
            {
                Write(processHandle, address + MarshalType<T>.Size*i, dataArray[i]);
            }
        }
