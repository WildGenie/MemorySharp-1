    ...
    public class AssemblyFactory : IFactory
    {
        ...
        public T Execute<T>(IntPtr address, CallingConventions callingConvention, params dynamic[] parameters)
        {
            ...
            var marshalledParameters =
                parameters.Select(p => MarshalValue.Marshal(MemorySharp.Handle, p)).Cast<IMarshalledValue>().ToArray();
            // Start a transaction
        ...
        {
            MemorySharp.WriteArray(address, Assembler.Assemble(asm, address), false);
        }
        ...
        public ProxyAllocation Inject(string asm)
        {
