...
namespace Binarysharp.MemoryManagement.Core.Marshaling
{
    ...
    public class MarshalledValue<T> : IMarshalledValue
    {
        ...
        #region IMarshalledValue Members
        ...
        private void Marshal()
        {
            ...
            else
            {
                ...
                var byteArray = RemoteMarshal<T>.ObjectToByteArray(Value);

                // If the value can be stored directly in registers
                if (RemoteMarshal<T>.CanBeStoredInRegisters)
                {
                    // Convert the byte array into a pointer
                    Reference = RemoteMarshal<IntPtr>.ByteArrayToObject(byteArray);
                }
                else
                {
                    ...
                    Allocated = MemorySharp.Memory.Allocate(RemoteMarshal<T>.Size);
                    // Write the value
