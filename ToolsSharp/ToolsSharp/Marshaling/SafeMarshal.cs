...
namespace ToolsSharp.Marshaling
{
    /// <summary>
    ...
    public static class SafeMarshal<T>
    {
        ...
        public static T ByteArrayToObject(byte[] byteArray)
        {
            ...
            using (var unmanaged = new LocalUnmanagedMemory(Size))
            {
                ...
                unmanaged.WriteBytes(byteArray);
        ...
        public static T PtrToObject(IntPtr handle, IntPtr pointer)
        {
            return ByteArrayToObject(CanBeStoredInRegisters
                ? BitConverter.GetBytes(pointer.ToInt64())
                : ExternalMemoryCore.ReadArray<byte>(handle, pointer, Size));
        }
    }
}